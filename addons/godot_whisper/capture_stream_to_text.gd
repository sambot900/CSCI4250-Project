# CaptureStreamToText.gd
# Node that transcribes real-time audio.
# Requires a bus with an AudioEffectCapture and a WhisperResource language model.

class_name CaptureStreamToText
extends SpeechToText

signal transcribed_msg(is_partial, new_text)

# For Traditional Chinese: "以下是普通話的句子。"
# For Simplified Chinese: "以下是普通话的句子。"
@export var initial_prompt: String

func _get_configuration_warnings():
	if language_model == null:
		return ["You need a language model."]
	else:
		return []

@export var recording := true:
	set(value):
		if recording == value:
			return  # No change, do nothing
		recording = value
		if recording:
			_start_thread()
		else:
			_stop_thread()
	get:
		return recording

# The interval at which transcribing is done. Use a value bigger than the time it takes to transcribe (depends on model).
@export var transcribe_interval := 0.3
# Using dynamic audio context speeds up transcribing but may result in mistakes.
@export var use_dynamic_audio_context := true
# How much time has to pass in seconds until we can consider a sentence.
@export var minimum_sentence_time := 3
# Maximum time a sentence can have in seconds.
@export var maximum_sentence_time := 15
# How many tokens it's allowed to hallucinate. Can provide useful info as it talks, but too much can provide useless text.
@export var hallucinating_count := 1
# The record bus has to have an AudioEffectCapture at index specified by audio_effect_capture_index.
@export var record_bus := "Record"
# The index where the AudioEffectCapture is located in the record_bus.
@export var audio_effect_capture_index := 0

# Characters to consider when ending a sentence.
@export var punctuation_characters := ".!?;。；？！"

@onready var _idx = AudioServer.get_bus_index(record_bus)
@onready var _effect_capture = AudioServer.get_bus_effect(_idx, audio_effect_capture_index) as AudioEffectCapture

var thread: Thread
var _accumulated_frames: PackedVector2Array = PackedVector2Array()

func _ready():
	if Engine.is_editor_hint():
		return
	if recording:
		_start_thread()

func _start_thread():
	if thread and thread.is_alive():
		_stop_thread()
	thread = Thread.new()
	_effect_capture.clear_buffer()
	thread.start(self.transcribe_thread)

func _stop_thread():
	if thread and thread.is_alive():
		recording = false  # Signal the thread to stop
		thread.wait_to_finish()
		thread = null  # Clear the thread reference

func _exit_tree():
	recording = false
	if thread and thread.is_alive():
		thread.wait_to_finish()
		thread = null

func transcribe_thread():
	var last_token_count = 0
	while recording:
		var start_time = Time.get_ticks_msec()
		var frames_available = _effect_capture.get_frames_available()
		if frames_available > 0:
			_accumulated_frames.append_array(_effect_capture.get_buffer(frames_available))
		else:
			OS.delay_msec(int(transcribe_interval * 1000))
			continue

		var resampled = resample(_accumulated_frames, SpeechToText.SRC_SINC_FASTEST)
		if resampled.size() <= 0:
			OS.delay_msec(int(transcribe_interval * 1000))
			continue

		var no_activity = voice_activity_detection(resampled)
		var total_time = resampled.size() / SpeechToText.SPEECH_SETTING_SAMPLE_RATE
		var audio_ctx = 0
		if use_dynamic_audio_context:
			audio_ctx = int(total_time * 1500 / 30 + 128)
		var tokens = transcribe(resampled, initial_prompt, audio_ctx)
		if tokens.is_empty():
			push_warning("No tokens generated")
			return

		var full_text = tokens.pop_front()
		var finish_sentence = false
		if total_time > maximum_sentence_time:
			finish_sentence = true
		var text = ""
		for token in tokens:
			text += token["text"]
		text = _remove_special_characters(text)
		if _has_terminating_characters(text, punctuation_characters) or no_activity:
			finish_sentence = true
		if total_time < minimum_sentence_time or abs(tokens.size() - last_token_count) > hallucinating_count:
			finish_sentence = false

		var time_processing = Time.get_ticks_msec() - start_time
		if not recording:
			break  # Exit the loop if recording is false

		call_deferred("emit_signal", "transcribed_msg", finish_sentence, full_text)
		last_token_count = tokens.size()
		print(full_text)
		print("Transcribe time: " + str(time_processing / 1000.0) + " s")

		if finish_sentence:
			var mix_rate = ProjectSettings.get_setting("audio/driver/mix_rate")
			_accumulated_frames = _accumulated_frames.slice(_accumulated_frames.size() - int(0.2 * mix_rate))

		# Sleep remaining time
		var interval_sleep = int(transcribe_interval * 1000 - time_processing)
		if interval_sleep > 0:
			OS.delay_msec(interval_sleep)

	# Thread is exiting
	print("Transcribe thread exited.")

func _has_terminating_characters(message: String, characters: String) -> bool:
	for character in characters:
		if message.contains(character):
			return true
	return false

func _remove_special_characters(message: String) -> String:
	var special_characters = [
		{ "start": "[", "end": "]" },
		{ "start": "<", "end": ">" },
		{ "start": "♪", "end": "♪" }
	]
	for special_character in special_characters:
		while message.find(special_character["start"]) != -1:
			var begin_index = message.find(special_character["start"])
			var end_index = message.find(special_character["end"], begin_index)
			if end_index != -1:
				message = message.substr(0, begin_index) + message.substr(end_index + 1)
			else:
				break  # No matching end character found
	var hallucination_phrases = [". you."]
	for phrase in hallucination_phrases:
		message = message.replace(phrase, "")
	return message

func _notification(what):
	if what == NOTIFICATION_PREDELETE:
		recording = false
		if thread and thread.is_alive():
			thread.wait_to_finish()
			thread = null
